...
+------------------+    +-------------------+    +--------+---------+
                                    ...
                                    |  + URL, grouped) |  +--------+---------+
                                    +--------+---------+           |
                                             |                     | news_index_YYYYMMDD_HHMMSS.json
                                             v                     v
                                    +-------------------+  +------------------+
                                    |  Email Sender     |  |  output/         |
...
**Output:** `news_index_YYYYMMDD_HHMMSS.json` saved to `output/`（例: `news_index_20260407_0934.json`）
...
- `write_index()` 内で自己決定: `session = "AM" if datetime.now().hour < 12 else "PM"`
- main.py からは session を受け取らない
- ファイル名: `date_str = now.strftime("%Y%m%d")`, `time_str = now.strftime("%H%M")`
...
- Sort by filename (lexicographic = chronological given `YYYYMMDD_HHMM` naming)
- If count > `index.max_files` (default: 3), delete oldest files until count == `max_files`
- This ensures the last `index.max_files` runs are always retained

<!-- DESIGN NOTE: ファイル名ソートの時系列一致の検証
`news_index_YYYYMMDD_HHMMSS.json` 形式は辞書順と時系列順が一致する。
例: `news_index_20260407_0800.json` < `news_index_20260407_1600.json`
のため FIFO の削除ロジックは正しく動作する。
...
translation:
  ...
  batch_interval_seconds: 15      # Interval between batches in seconds (default: 15)
